...
/mob/living/simple_animal/hostile/asteroid/wolf/alpha/Initialize(mapload)
	. = ..()
	charge = new /datum/action/cooldown/mob_cooldown/charge/basic_charge()
	charge.Grant(src)
	charge.cooldown_time = 20
	charge.charge_distance = 7
...
/mob/living/simple_animal/hostile/asteroid/wolf/alpha/Destroy()
	QDEL_NULL(charge)
	return ..()
	
/mob/living/simple_animal/hostile/asteroid/wolf/alpha/OpenFire()
	if(client)
		return
