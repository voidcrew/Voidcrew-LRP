	...
	for(var/I in currentrun)
		...
		if(round_started && kick_inactive && !C.holder && C.is_afk(afk_period))
			...
			if (!isnewplayer(cmob) || !SSticker.queued_players.Find(cmob))
				...
				to_chat(C, "[span_userdanger("You have been inactive for more than [DisplayTimeText(afk_period)] and have been disconnected.")]<br>[span_danger("You may reconnect via the button in the file menu or by <b><u><a href='byond://winset?command=.reconnect'>clicking here to reconnect</a></u></b>.")]")
